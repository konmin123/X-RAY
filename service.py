    ...
    def threading_request(cls):
        ...
        for exp in cls.__exp:
            for page in range(0, 20):
                th = threading.Thread(target=cls.get_page, args=(cls.__search_word[0], exp, page))
                th.start()
            while True:
...
class Analizer:
    ...
    def squeezer(cls, vac):
        ...
        if vac['salary']:
            if vac['salary']['currency'] in ['RUR', 'USD', 'EUR']:
                ...
                if vac['salary']['from']:
                    raw['sum_min_salary'] += vac['salary']['from'] * CurrencyConverter.exchange_rate(cur)
                    raw['count_min_salary'] += 1
                if vac['salary']['to']:
                    raw['sum_max_salary'] += vac['salary']['to'] * CurrencyConverter.exchange_rate(cur)
    ...
    def create_table(cls, word):
        ...
        Analizer.slicer()
        print(cls.data)
        df = pandas.DataFrame({'search_word': cls.data['language_db'],
                               'experience': cls.data['experience_db'],
                               'quantity_vac': cls.data['count_vac_db'],
                               'avarage_salary': cls.data['ave_salary_db'],
                               'number_of_vacancies_for_avarage_salary': cls.data['count_for_ave_sal_db'],
...
if __name__ == '__main__':
    ...
    Analizer.create_table('повар')
    toc = time.perf_counter()
    print(f"Вычисление заняло {toc - tic:0.1f} секунд")
